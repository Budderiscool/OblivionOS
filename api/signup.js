...
// In-memory user store (demo). Replace with persistent DB for real product.
const USERS = global.__OBLIVION_USERS ||= {};

// secret for signing JWTs - set process.env.JWT_SECRET in production
...
export default async function handler(req, res) {
  ...
  try {
    ...
    USERS[username] = { username, passwordHash: hashed, createdAt: Date.now() };

    // issue token in cookie
